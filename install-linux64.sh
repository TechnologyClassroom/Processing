...
# This script installs the latest version of Processing for GNU/Linux 64bit into /usr/local/lib
# Run it like this: "sudo install-linux64.sh"

# this assumes that newer releases are at the top
TAR="$(curl -sL https://api.github.com/repos/processing/processing/releases | grep -oh -m 1 'https.*linux64.tgz')"

echo "\nDownloading $TAR..."
curl -L $TAR > processing-linux64-latest.tgz

echo "Installing in /usr/local..."
tar fx processing-linux64-latest.tgz -C /usr/local/lib
rm -f processing-linux64-latest.tgz
